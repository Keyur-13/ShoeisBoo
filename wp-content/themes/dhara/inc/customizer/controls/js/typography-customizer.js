( function( api ) {
	...
	api.controlConstructor['dhara-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.dhara-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the dhara_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'dhara_get_all_google_fonts_ajax',
								dhara_customize_nonce: dhara_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.dhara-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.dhara-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.dhara-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.dhara-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.dhara-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.dhara-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.dhara-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.dhara-font-variant' );
			...
			control.container.on( 'change', '.dhara-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.dhara-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.dhara-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.dhara-font-family select' ).selectWoo();
	jQuery( '.dhara-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".dhara-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.dhara-font-variant' ).hide();
		}
	});
