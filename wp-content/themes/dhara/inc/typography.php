...
if ( ! function_exists( 'dhara_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'dhara_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function dhara_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', dhara_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $dhara_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $dhara_settings[$key], ':' ) !== false ) {
					$dhara_settings[$key] = current( explode( ':', $dhara_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $dhara_settings[$key] );

				// Grab the variants using the plain name
				$variants = dhara_get_google_font_variants( $dhara_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'dhara_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'dhara_fonts_subset','' );
		...
		$default_fonts = DHARA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('dhara-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('dhara-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'dhara_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'dhara_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function dhara_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'dhara_fonts_customize_register' ) ) {
			// Bail if Dhara Premium is active
			return;
		}
		...
		$defaults = dhara_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Dhara_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'dhara_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'dhara_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'dhara_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Dhara_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'dhara' ),
					'settings' => array(
						'family' => 'dhara_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'dhara_settings[body_font_weight]',
						'transform' => 'dhara_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Dhara_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'dhara' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'dhara_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'dhara_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'dhara_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Dhara_Range_Slider_Control(
				...
				array(
					'type' => 'dhara-range-slider',
					'description' => __( 'Line height', 'dhara' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'dhara_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'dhara_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Dhara_Range_Slider_Control(
				...
				array(
					'type' => 'dhara-range-slider',
					'description' => __( 'Paragraph margin', 'dhara' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'dhara_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'dhara_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function dhara_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'dhara_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'dhara_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'dhara_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_dhara_get_all_google_fonts_ajax', 'dhara_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function dhara_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'dhara_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'dhara_customize_nonce' ] ), 'dhara_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'dhara_customize_nonce', 'dhara_customize_nonce' );
		...
		$fonts = dhara_get_all_google_fonts();
...
if ( ! function_exists( 'dhara_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: dhara_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function dhara_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, dhara_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'dhara_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'dhara_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: dhara_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function dhara_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'dhara_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function dhara_get_font_family_css( $font, $settings, $default ) {
		$dhara_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'dhara_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $dhara_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'dhara_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . dhara_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'dhara_add_to_font_customizer_list' ) ) {
	add_filter( 'dhara_typography_customize_list', 'dhara_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function dhara_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = dhara_get_all_google_fonts();
		$select_fonts = dhara_get_all_google_fonts( apply_filters( 'dhara_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $dhara_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $dhara_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $dhara_settings[ $setting ], dhara_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $dhara_settings[ $setting ] ) ) ] = array(
				'name' => $dhara_settings[ $setting ],
		...
		if ( function_exists( 'dhara_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], dhara_typography_default_fonts() ) ) {
