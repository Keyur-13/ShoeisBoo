...
add_action( 'after_setup_theme', 'dhara_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function dhara_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'dhara_construct_sidebars' );
}

if ( ! function_exists( 'dhara_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'dhara_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function dhara_woocommerce_start() { ?>
...
if ( ! function_exists( 'dhara_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'dhara_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function dhara_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'dhara_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'dhara_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function dhara_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'dhara_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'dhara_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'dhara_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function dhara_bbpress_css() {
...
if ( ! function_exists( 'dhara_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'dhara_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function dhara_buddypress_css() {
...
if ( ! function_exists( 'dhara_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'dhara_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function dhara_beaver_builder_css() {
