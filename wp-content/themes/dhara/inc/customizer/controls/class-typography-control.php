...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Dhara_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Dhara_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'dhara-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'dhara-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), DHARA_VERSION, true );
			wp_enqueue_style( 'dhara-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), DHARA_VERSION );

			wp_enqueue_script( 'dhara-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'dhara-typography-selectWoo' ), DHARA_VERSION, true );
			wp_enqueue_style( 'dhara-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), DHARA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'dhara_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'dhara' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'dhara' );
			$this->json[ 'google_fonts' ] = apply_filters( 'dhara_typography_customize_list', dhara_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = dhara_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'dhara' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'dhara' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'dhara' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'dhara' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="dhara-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="dhara-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="dhara-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="dhara-font-transform">
					<label>
